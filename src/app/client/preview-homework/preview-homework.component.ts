...
@Component({
  selector: 'app-preview-homework',
  templateUrl: './preview-homework.component.html',
  styleUrls: ['./preview-homework.component.less'],
  providers: [ClientCourseVideoService]
})
export class PreviewHomeworkComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if((this.allUrl.indexOf("testId"))>-1){
      ...
      this.paperMarkingService.previewTestPaper(this.testId).subscribe(result=>{
        ...
        console.log("--------作业-----------",this.testPaperContent);
        this.dealWithData();
      });
    } else {
  ...
  goBack() {
    if (location.pathname.endsWith('preview_homework')) {
      history.go(-2)
    } else {
      history.go(-1)
    }
